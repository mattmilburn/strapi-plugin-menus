...
import { getService, sanitizeEntity } from '../utils';

export type MenuItemService = ReturnType<typeof menuItemService>;
      ...
      let createdItems = await Promise.all(
        itemsToCreate.map((item) => {
          ...
          const sanitizedItem = sanitizeEntity(
            item.parent && typeof item.parent.id === 'string' ? omit(item, 'parent') : item
          );
