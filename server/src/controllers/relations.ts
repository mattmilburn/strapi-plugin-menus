...
// import { getService } from '../utils'; // CUSTOM MOD [4].
import { validateFindAvailable, validateFindExisting } from './validation/relations';
import { isListable } from '../utils'; // CUSTOM MOD [3].
...
const relationsController = {
  async findAvailable(ctx) {
    ...
    if (contentTypes.hasDraftAndPublish(targetedModel)) {
      fieldsToSelect.push(contentTypes.constants.PUBLISHED_AT_ATTRIBUTE);
    }
