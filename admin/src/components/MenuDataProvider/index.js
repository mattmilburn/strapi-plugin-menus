...
const MenuDataProvider = ({children, isCreatingEntry, menu}) => {
  ...
  const deleteMenuItem = (id) => {
    ...
    let newItems = values.items.filter((item) => {
      ...
      const isDescendant = descendantsToDelete.find(
    ...
    const orderedSiblings = sortByOrder(siblings).map((item, order) => ({
      ...item,
      order,
    }));
    ...
    newItems = newItems.map((item) => {
      const reorderedItem = orderedSiblings.find(
        (_item) => _item.id === item.id
      );
  ...
  useEffect(() => {
    ...
    const newActiveItem = get(values, "items", []).find(
      ({order, parent}) =>
        order === activeMenuItem.order && parent === activeMenuItem.parent
    );
